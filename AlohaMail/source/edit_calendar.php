...
include('../include/html_forms.inc');
...
if (isset($date)){
	...
	if ($edit>0){
    	...
    	$backend_query = "SELECT * FROM $DB_CALENDAR_TABLE WHERE (userID='$session_dataID' or shared=1) and id='$edit'";
		...
		if (($backend_result) && ($db->num_rows($backend_result)>0)){
			...
			//echo $error;
			$error.="Invalid item, or access denied";
			$edit="";
		}
	}else{
		$publish = 1;
...
<?php
	if ($error) echo '<p><span class="error">'.$error.'</span>';
?>
...
<table>
	<tr>
		...
		while ( list($value,$label)=each($cal_colors) ){
			$style='style="color:'.$value.';background-color:'.$cal_bg_colors[$value].';"';
			echo '<option value="'.$value.'" '.$style.' '.($value==$color?"SELECTED":"").'>'.$calStr["color_names"][$label]."\n";
		}
		?>
		</select>
		</td>
	...
	<tr>
		...
		<td class=mainLight align="left" valign="top">
			<?php
				$temp = $lang_datetime['prevyears'];
				$temp = str_replace('%y',input_text('start_year',$start_year,4), $temp);
				$temp = str_replace('%m',input_text('start_month',$start_month,2),$temp);
				$temp = str_replace('%d',input_text('start_day',$start_day,2),$temp);
				echo $temp;
			?>
			<!--
		...
		<td class=mainLight align="left" valign="top">
			<?php
				$temp = $lang_datetime['prevyears'];
				$temp = str_replace('%y',input_text('end_year',$end_year,4), $temp);
				$temp = str_replace('%m',input_text('end_month',$end_month,2),$temp);
				$temp = str_replace('%d',input_text('end_day',$end_day,2),$temp);
				echo $temp;
			?>
			<!--
	...
	<tr>
		...
		<label><input type="checkbox" name="allday" value="1" <?php echo ($allday?'CHECKED':'')?>>
		<?php echo $calStr['is all day']?></label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class=mainLight align="left" valign="top">
			<label><input type="checkbox" name="repeat_d[0]" value=1 <?php echo ($repeat_d[0]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][0]?></label>
			<br><label><input type="checkbox" name="repeat_d[1]" value=1 <?php echo ($repeat_d[1]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][1]?></label>
			<br><label><input type="checkbox" name="repeat_d[2]" value=1 <?php echo ($repeat_d[2]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][2]?></label>
			<br><label><input type="checkbox" name="repeat_d[3]" value=1 <?php echo ($repeat_d[3]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][3]?></label>
			<br><label><input type="checkbox" name="repeat_d[4]" value=1 <?php echo ($repeat_d[4]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][4]?></label>
			<br><label><input type="checkbox" name="repeat_d[5]" value=1 <?php echo ($repeat_d[5]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][5]?></label>
			<br><label><input type="checkbox" name="repeat_d[6]" value=1 <?php echo ($repeat_d[6]?"CHECKED":"") ?>><?php echo $lang_datetime["dsowl"][6]?></label>
		</td>
		<td class=mainLight align="right" valign="top"><?php echo $calStr["of"]?></td>
		<td class=mainLight align="left" valign="top">
			<label><input type="checkbox" name="repeat_w[1]" value="1" <?php echo ($repeat_w[1]?"CHECKED":"") ?>><?php echo $calStr["weeks"][1]?></label>
			<br><label><input type="checkbox" name="repeat_w[2]" value="1" <?php echo ($repeat_w[2]?"CHECKED":"") ?>><?php echo $calStr["weeks"][2]?></label>
			<br><label><input type="checkbox" name="repeat_w[3]" value="1" <?php echo ($repeat_w[3]?"CHECKED":"") ?>><?php echo $calStr["weeks"][3]?></label>
			<br><label><input type="checkbox" name="repeat_w[4]" value="1" <?php echo ($repeat_w[4]?"CHECKED":"") ?>><?php echo $calStr["weeks"][4]?></label>
	...
	<tr>
		<td class=mainLight align="right" valign="top"></td>
		<td class=mainLight align="left" valign="top" colspan="3">
			<label><input type="checkbox" name="repeat_monthly" value=1 <?php echo ($repeat_monthly?"CHECKED":"") ?>><?php echo $calStr["monthly"]?></label>
			<br><label><input type="checkbox" name="repeat_yearly" value=1 <?php echo ($repeat_yearly?"CHECKED":"") ?>><?php echo $calStr["yearly"]?></label>
		</td>
	</tr>
	<tr>
		<td class=mainLight align="right" valign="top">&nbsp;</td>
	...
	<?php
	if (!$DISABLE_CALENDAR_SHARING){
	?>
	<tr>
		<td class=mainLight align="right" valign="top">
		</td>
		<td class=mainLight align="left" valign="top" colspan="3">
			<label><input type="checkbox" name="shared" value="1" <?php echo ($shared?"CHECKED":"") ?>></lablel>
			<?php echo $calStr["public"]?>
		</td>
	</tr>
	<tr>
		<td class=mainLight align="right" valign="top">
		...
		<td class=mainLight align="left" valign="top" colspan="3">
			<label><input type="checkbox" name="publish" value="1" <?php echo ($publish?"CHECKED":"") ?>></lablel>
			<?php echo $calStr["publish"]?>
		</td>
	</tr>
	<?php
	}
...
<?php 
if (!$userID || $userID==$session_dataID){
?>
	<table width="50%"><tr>
