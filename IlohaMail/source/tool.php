...
<?php
/*** include CSS ***/
include('themes/'.$my_prefs['theme'].'/css_tool.php');
?>
</style>
</HEAD>
<?php 
...
if (($my_prefs["list_folders"])||(!$ICL_CAPABILITY["folders"])){
	$links[] = array("main.php?folder=INBOX&user=$user&cmc=1", "list2", $toolStrings["inbox"], "&nbsp;&nbsp;&nbsp;&nbsp;");
...
?>
<table width="100%"><tr class="menuText"><td>
<?php

if (!empty($THEME_VARS['logo'])) {
	echo '<div id="logo"><a href="'.$THEME_VARS['logo_href'].'" target="_parent"><img src="'.$THEME_VARS['logo'].'" alt="'.$THEME_VARS['logo_alt'].'" title="'.$THEME_VARS['logo_alt'].'"></a></div></td><td>';
}
