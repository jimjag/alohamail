...
// +----------------------------------------------------------------------+ 
// 
// $Id: Contact_Vcard_Parse.php,v 1.1 2004/08/16 08:49:14 ryochiji Exp $ 
...
class Contact_Vcard_Parse //extends PEAR {
    ...
    */
    ...
    function fromText($text, $decode_qp = true)
    {
        ...
        // line 2 starts with any amount of whitespace.
        $fold_regex = '(\n)([ |\t]+)';
    ...
    */
    ...
    /**
    * 
    * Used to make string human-readable after being a vCard value. DO
    * NOT use this with binary encodings.
    * 
    * Converts...
    *     \; => ;
    *     \, => ,
        ...
        foreach ($source as $line) {
            ...
            if (! $begin) {
                ...
                if (strtoupper($left) == 'END' &&
                    ...
                    default:
                        // by default, just grab the plain value.
                        // keep as an array to make sure *all*
                        // values are arrays.  for consistency. ;-)
                        $value = array($right);
                        break;
                    }
	...
	* @access private
	* 
	* @param string $value The first element in a
	* parameter-name/parameter-value pair.
	* 
	* @return string The proper parameter name.
	* 
    */
    ...
    */
    ...
    function _parseN($text)
    {
        ...
        return array(
            'family_name' => $this->splitByComma($tmp[0]),
            'given_name' => $this->splitByComma($tmp[1]),
            'additional_names' => $this->splitByComma($tmp[2]),
            'honorific_prefix' => $this->splitByComma($tmp[3]),
            'honorific_suffix' => $this->splitByComma($tmp[4])
        );
    }
    ...
    */
    ...
    function _parseADR($text)
    {
        $tmp = $this->splitBySemi($text);
        return array(
            'pob' => $this->splitByComma($tmp[0]),
            'extended' => $this->splitByComma($tmp[1]),
            'street' => $this->splitByComma($tmp[2]),
            'locality' => $this->splitByComma($tmp[3]), // city
            'region' => $this->splitByComma($tmp[4]), // state
            'postcode' => $this->splitByComma($tmp[5]), // ZIP
            'country' => $this->splitByComma($tmp[6])
        );
    }
