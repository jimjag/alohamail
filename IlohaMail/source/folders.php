...
function getFolderStates(){
	global $loginID, $host, $CACHE_ERROR,$SUBSCRIBED;
...
function saveFolderStates($folders){
	global $loginID, $host, $SUBSCRIBED;
...
if (isset($user)){
	...
	include_once('../include/stack.inc');
	include_once('../include/cache.inc');

	$bodyString = '<body class="folders">'."\n";
?>
<HTML <?php echo ($lang_rtl[$my_prefs['lang']]?'dir="rtl"':'')?>>
<HEAD>
	...
	include('themes/'.$my_prefs['theme'].'/css_folders.php');
	echo '</style>'."\n";
	?>
</HEAD>
<?php
	...
	echo '<div id="folders"><a href="folders.php?user='.$user.'"><b>'.$fl_str['folders'].'</b></a></div>'."\n";
	echo '<div id="manage"><a href="edit_folders.php?user='.$user.'" target="list2">['.$fl_str['manage'].']</a></div>';
	echo '<br>';

	// If showing unread count, run filters on INBOX
	if ($my_prefs["showNumUnread"] && !(isset($dofilter) && $dofilter == 'no')) {
	...
	if (!is_array($folders)){
		...
		else $subscribed = getFolderStates();
		...
		reset ($defaults);
 		while (list($key, $value) = each ($defaults)) {
			if (($value!=".")&&(!empty($key))){
				...
				echo "<a href=\"main.php?folder=$key&user=".$user."&cmc=1\" target=\"list2\">$value $unseen_str</a>";
        ...
        while ( list($path, $display) = each($result) ){
            if ((!empty($display)) && (($containers[$path])||(empty($defaults[$path])))){
				...
				echo '<span class="spacer"><tt>'.$indent[$key].'</tt></span>';
                echo $button;
                ...
                $path = urlencode($path);				
				echo "<a href=\"main.php?folder=$path&user=".$user."&cmc=1\" target=\"list2\">".$display.$unseen_str."</a><BR>\n";
				flush();
            }
        }
	}
